...
### 4) CI 收尾阶段（SLO）
- 已定义并落地 4 个 SLO 指标：CI 通过率、平均时长、Flaky rate、非白名单 skip 数。
- 最近 14 天趋势已登记到 `docs/ci-slo.md`（当前仓库快照缺少可查询远端 CI 历史，趋势表先按同口径占位，待看板回填）。
- 达标线（连续 7 天）已设定：
  - 通过率 ≥ 98%
  - 非白名单 skip = 0
  - flaky ≤ 1%
- 当前状态：**待观测（Insufficient CI telemetry）**。
- 验收口径：满足达标线即认定“最后 2.65% 完成”。

...
1. `DegradeRateHigh`、`HighDegradeRatio` 等主告警已恢复（或进入恢复态信息告警）。
2. `Degrade Rate (5m)` 回落并稳定在阈值以下（建议持续 30 分钟观察）。
3. 主导 reason 占比恢复正常（无单点原因持续放大）。
4. 主模型链路恢复，fallback 仅偶发触发，且 `FallbackErrorSpike` 不再触发。
5. 已补充事故记录（时间线、根因、修复与预防动作）。
...
- 主导模型 / 主导 reason：
- 已执行动作：
- 当前状态（Detect/Triage/Mitigate/Recover/Review）：
- 待跟进项与 ETA：

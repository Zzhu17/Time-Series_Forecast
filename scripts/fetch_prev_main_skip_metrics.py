...
def download_with_redirect(url: str, headers: dict[str, str]) -> bytes:
    req = urllib.request.Request(url, headers=headers)
    try:
        with urllib.request.urlopen(req, timeout=60) as resp:
            return resp.read()
    except urllib.error.HTTPError as exc:
        # archive_download_url commonly returns 302. Some environments lose auth
        # headers on cross-host redirects; retry with redirected URL and no auth.
        if exc.code in (301, 302, 303, 307, 308):
            redirected = exc.headers.get("Location")
            if redirected:
                bare_req = urllib.request.Request(redirected)
                with urllib.request.urlopen(bare_req, timeout=60) as resp:
                    return resp.read()
        raise
...
def main() -> int:
    ...
    try:
        runs = get_json(
            f"{api}/actions/workflows/ci.yml/runs?branch=main&status=success&per_page=10",
            headers,
        )
        candidate = None
        ...
        target = next(
            (a for a in artifacts.get("artifacts", []) if a.get("name") == "minimal-skip-metrics"),
            None,
        )
        if not target:
            write_fallback()
        ...
        blob = download_with_redirect(target["archive_download_url"], headers)
        zip_path = Path("prev-main-artifact.zip")
        ...
        with zipfile.ZipFile(zip_path) as zf:
            ...
            if not json_members:
                ...
                return 0
            content = zf.read(json_members[0]).decode("utf-8")
        ...
        # Do not fail the build for auth/permission differences on forks.
